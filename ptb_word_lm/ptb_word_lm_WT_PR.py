...
class PTBModel(object):
  ...
  def __init__(self, is_training, config):
    ...
    proj = tf.get_variable("proj", [size,size])
    proj_b = tf.get_variable("proj_b", [size])
    ...
    l2loss = tf.nn.l2_loss(proj) / (batch_size*6.5)
    self._cost =  tf.reduce_sum(loss) / batch_size
    cost = tf.reduce_sum(loss) / batch_size + l2loss
