-- This script implements Bayesian LSTM (Gal, 2015) for the large model of Zaremba et al. (2014).
...
-- are 0.3 and 0.5 compared Zaremba et al.'s 0.6).
-- 
-- Single model test perplexity is improved from Zaremba et al.'s 78.4 to 76.5.
-- Validation perplexity is reduced from 82.2 to 79.1, see attached log.
-- 
-- References:
...
local state_train, state_train_check, state_valid, state_test
local model = {}
...
local function create_network()
  ...
  local lt               = LookupTable(params.vocab_size,
                                              params.rnn_size)(x)
  local i                = {[0] = lt}
  ...
  h2y.weight:copy(lt.data.module.weight:t())
  h2y:share(lt.data.module,'weight','gradWeight')
...
local function setup()
  ...
  for d = 1, params.layers do
    ...
    -- Yarin: tie noise mask for all gates (for efficiency - 65% quicker, but worse results)
  ...
  -- Yarin: For MC dropout we want to get pred as model output rather than the negative log probs (?)
  model.pred = {}
  for j = 1, params.seq_length do
...
local function run_train_check()
  reset_state(state_train_check)
  disable_dropout = true
  local len = (state_train_check.data:size(1) - 1) / (params.seq_length)
  local perp = 0
  for i = 1, len do
    local p = fp(state_train_check)
    perp = perp + p:mean()
  end
  print("Final train set perplexity : " .. g_f3(torch.exp(perp / len)))
...
local function main()
  ...
  state_train_check = {data=transfer_data(ptb.traindataset(params.batch_size))}
  state_valid =  {data=transfer_data(ptb.validdataset(params.batch_size))}
  state_test =  {data=transfer_data(ptb.testdataset(params.batch_size))}
  print("Network parameters:")
  print(params)
  local states = {state_train, state_train_check, state_valid, state_test}
